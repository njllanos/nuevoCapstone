...
enfermeros_df  = pd.read_excel("Capstone_Instancia/enfermeros_small.xlsx", sheet_name="MIERCOLES") #QUE COINCIDA EL DÍA DE LA SEMANA CON LOS PACIENTES
...
for _, row in pacientes_df.iterrows():
    atender = row['MIERCOLES'] #VEO QUÉ DÍA DE LA SEMANA QUIERO OPTIMIZAR
    if atender == 'Si':
        id = row['ID']
...
for _, row in enfermeros_df.iterrows():
    ...
    inicio_str = row['HORARIO ENTRADA 1']
    fin_str = row['HORARIO SALIDA 1']
    
    # Extraer horas y minutos
    h_inicio, m_inicio = map(int, inicio_str.split(":"))
    h_fin, m_fin = map(int, fin_str.split(":"))
    
    # Convertir a decimal (ej. 8.5 para 08:30)
    inicio_decimal = h_inicio + m_inicio / 60
    fin_decimal = h_fin + m_fin / 60
    
    K[id] = {
        'regimen': row['REGIMEN'],
        'inicio_ventana': inicio_decimal * 60,
        'fin_ventana'   : fin_decimal * 60
    }
...
#Se encuuentra el primer enfermero de cada turno y tipo
k1_min = min(K_subsets['K1'])
k2_min = min(K_subsets['K2'])
kex_min = min(K_subsets['KEX'])
...
for n in N_ids:
    ...
    if id > 1000 and id < 2000: # es un paciente
        ...
        a[n] = max(a[10] + t[10,n], a[n])
        b[n] = int(P[id - 1000]["fin_ventana"]) * 60
        b[n] = min(b[n], b[2] - t[n,2])
...
Mgrande_i = {i: a[i] for i in N_ids} #Definir un M personalizado para cada i
m.addConstrs(
    (a[i] - Mgrande_i[i] * (1 - Z[k, i]) <= I[i, k]
      for i in N_ids
      for k in K_ids),
    name="R12.1"
)

Mgrande_i = {i: b[i] for i in N_ids} #Definir un M personalizado para cada i
m.addConstrs(
    (I[i, k] <= b[i] + Mgrande_i[i] * (1 - Z[k, i])
      for i in N_ids
      for k in K_ids),
    name="R12.2"
)
...
#Secuencia temporal
Mgrande_kij = {(k,i,j): s[i] + t[(i,j)] + K[k - 5000]['fin_ventana'] for (i, j) in arcos for k in K_ids} #Definir un M personalizado para cada k,i,j
m.addConstrs(
    (I[j, k] >= I[i, k] + s[i] + t[(i,j)] - Mgrande_kij[k,i,j] * (1 - X[k, i, j])
      for (i, j) in arcos
      for k in K_ids 
...
#Definir Y
hospitales = {10, 20, 30, 1, 2, 3}
Ngrande = len(set(N_ids) - hospitales)
m.addConstrs(
...
M = 1380 #tiempo en que cierra el hospital
...
# Restricciones de Simetría Tipo 1

#Enfermeros turno 1
m.addConstrs(
    (gp.quicksum(X[k, 10, j] for j in N_ids if j not in [10, 20, 30, 1, 2, 3]) <= gp.quicksum(X[k - 1, 10, j] for j in N_ids if j not in [10, 20, 30, 1, 2, 3])
    for k in K_subsets['K1'] if k != k1_min), 
    name="R18.1"
)

#Enfermeros turno 2
m.addConstrs(
    (gp.quicksum(X[k, 20, j] for j in N_ids if j not in [10, 20, 30, 1, 2, 3]) <= gp.quicksum(X[k - 1, 20, j] for j in N_ids if j not in [10, 20, 30, 1, 2, 3])
    for k in K_subsets['K2'] if k != k2_min),
    name="R18.2"
)

#Enfermeros Externos
m.addConstrs(
    (gp.quicksum(X[k, 30, j] for j in N_ids if j not in [10, 20, 30, 1, 2, 3]) <= gp.quicksum(X[k - 1, 30, j] for j in N_ids if j not in [10, 20, 30, 1, 2, 3])
    for k in K_subsets['KEX'] if k != kex_min),
    name="R18.3"
)


# Restricciones de Simetría Tipo 2

#Enfermeros turno 1
m.addConstrs(
    (Z[k, i] <= gp.quicksum(Z[k - 1, j] for j in N_ids if j < i)
    for k in K_subsets['K1'] if k != k1_min
    for i in N_ids if i not in [10, 20, 30, 1, 2, 3]),
    name="R19.1"
)
...
m.setObjective(
    ...
    gp.quicksum(Z[k, i] * CEV['MIERCOLES'][P[i - 1000]['requerimiento']] for k in K_subsets['KEX'] for i in P_ids)
...
# Cargar solución inicial para X
X_start = {}
with open("Caso base/resultados/csv/X.csv", newline='') as f:
    reader = csv.DictReader(f)
    for row in reader:
        k = int(row["Enfermero"])
        i = int(row["Desde"])
        j = int(row["Hasta"])
        X_start[k, i, j] = int(row["Valor"])

# Cargar solución inicial para Z
Z_start = {}
with open("Caso base/resultados/csv/Z.csv", newline='') as f:
    reader = csv.DictReader(f)
    for row in reader:
        k = int(row["Enfermero"])
        i = int(row["Nodo"])
        Z_start[k, i] = int(row["Valor"])

# Cargar I
I_start = {}
with open("Caso base/resultados/csv/I.csv", newline='') as f:
    reader = csv.DictReader(f)
    for row in reader:
        i = int(row["Nodo"])
        k = int(row["Enfermero"])
        val = float(row["InicioAtencion"])
        I_start[i, k] = val

# Cargar Y
Y_start = {}
with open("Caso base/resultados/csv/Y.csv", newline='') as f:
    reader = csv.DictReader(f)
    for row in reader:
        k = int(row["Enfermero"])
        Y_start[k] = int(row["Externo"])

# Cargar RM
RM_start = {}
with open("Caso base/resultados/csv/RM.csv", newline='') as f:
    reader = csv.DictReader(f)
    for row in reader:
        k = int(row["Enfermero"])
        RM_start[k] = int(row["RM"])

# Cargar RE
RE_start = {}
with open("Caso base/resultados/csv/RE.csv", newline='') as f:
    reader = csv.DictReader(f)
    for row in reader:
        k = int(row["Enfermero"])
        RE_start[k] = int(row["RE"])

# Cargar W
W_start = {}
with open("Caso base/resultados/csv/W.csv", newline='') as f:
    reader = csv.DictReader(f)
    for row in reader:
        k = int(row["Enfermero"])
        W_start[k] = float(row["Espera"])


# Asignar solución inicial a las variables (con 0 por defecto donde falte)

for k in K_ids:
    for (i, j) in arcos:
        X[k, i, j].start = X_start.get((k, i, j), 0)

for k in K_ids:
    for i in N_ids:
        Z[k, i].start = Z_start.get((k, i), 0)

for i in N_ids:
    for k in K_ids:
        I[i, k].start = I_start.get((i, k), 0.0)

for k in K_ids:
    Y[k].start = Y_start.get(k, 0)
    RM[k].start = RM_start.get(k, 0)
    RE[k].start = RE_start.get(k, 0)
    W[k].start = W_start.get(k, 0.0)
...
if m.status == gp.GRB.INFEASIBLE:
    print("⚠️  Modelo inviable. Buscando IIS...")
    m.computeIIS()

    for constr in m.getConstrs():
        if constr.IISConstr:
            print(f"❌ Restricción en IIS: {constr.ConstrName}")

    for var in m.getVars():
        if var.IISLB:
            print(f"⬇️ Límite inferior de variable en IIS: {var.VarName}")
        if var.IISUB:
            print(f"⬆️ Límite superior de variable en IIS: {var.VarName}")

    for sos in m.getSOSs():
        if sos.IISSOS:
            print(f"🔗 SOS en IIS: {sos.getAttr('Name')}")

    print("🔎 Usa esta información para revisar tus restricciones más conflictivas.")
...
if m.status not in (GRB.OPTIMAL, GRB.TIME_LIMIT, GRB.SUBOPTIMAL):
    ...
    var_ext = sum(
        Z[k, i].X * CEV['MIERCOLES'][ P[i-1000]['requerimiento'] ]
        for k in K_subsets['KEX']
        for i in P_ids
    )
