...
# Crear ExcelWriter para las 12 semanas (arcos y resumen)
with pd.ExcelWriter("resultados/arcos.xlsx") as writer_arcos, pd.ExcelWriter("resultados/arcos_resumen.xlsx") as writer_resumen:

    for semana in range(1, 13):
        print(f"\n=======================")
        print(f"Procesando semana s{semana}...")
        print(f"=======================")
        ...
        pacientes = pd.read_excel("Capstone_Instancia/data/pacientes.xlsx", sheet_name=f"s{semana}")
        ...
        #Crear carpeta de arcos dentro de resultados si no existe
        if not os.path.exists("resultados/arcos"):
            os.makedirs("resultados/arcos")
        #Crear carpeta de semana "semana" dentro de resultados/arcos si no existe
        if not os.path.exists(f"resultados/arcos/s{semana}"):
            os.makedirs(f"resultados/arcos/s{semana}")

        gdf.to_file(f"resultados/arcos/s{semana}/puntos_con_nodos.geojson", driver="GeoJSON")
        ...
        df_arcos.to_excel(writer_arcos, sheet_name=f"s{semana}", index=False)
        df_arcos.to_excel(f"resultados/arcos/s{semana}/arcos.xlsx", index=False)

        df_resumen = df_arcos[["id_origen","id_destino", "tiempo_min", "distancia_m"]]
        df_resumen.to_excel(writer_resumen, sheet_name=f"s{semana}", index=False)
        df_resumen.to_excel(f"resultados/arcos/s{semana}/arcos_resumen.xlsx", index=False)
